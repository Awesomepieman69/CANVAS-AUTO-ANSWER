document.addEventListener('DOMContentLoaded', () => {
    ...
    const creamAllButton = document.getElementById('creamAllButton');
    ...
    creamAllButton.addEventListener('click', () => {
      chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
        if (tabs.length > 0) {
          const tabId = tabs[0].id;
          updateStatus('Starting Cream Key Cremate...');
          creamAllButton.disabled = true;
          chrome.tabs.sendMessage(
            tabId,
            { action: 'creamAllQuestions' },
            response => {
              if (chrome.runtime.lastError) {
                updateStatus(`Error: ${chrome.runtime.lastError.message}`, true);
              } else if (response && response.status === 'completed') {
                updateStatus('All questions creamed!');
              } else if (response && response.status === 'error') {
                updateStatus(`Error: ${response.message || 'Unknown error during cream all.'}`, true);
              } else {
                updateStatus('Processing...');
              }
              setTimeout(() => { creamAllButton.disabled = false; }, 3000);
            }
          );
        } else {
          updateStatus('Could not find active tab.', true);
        }
      });
    });
